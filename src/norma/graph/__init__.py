"""
Norma LangGraph pipeline — T1 → T2 → T3 → T4 → emit or revise.

Graph:
  intake_node → gherkin_specialist_node → nfr_specialist_node → cai_gate_node
                      ↑                                               |
                      └────────────── revise ─────────────────────────┘
                                                                       |
                                                                  end / halt → END
...
def build_graph() -> StateGraph[NormaState]:
    g = StateGraph(NormaState)

    g.add_node("intake", intake_node)
    g.add_node("gherkin_specialist", gherkin_specialist_node)
    g.add_node("nfr_specialist", nfr_specialist_node)
    g.add_node("cai_gate", cai_gate_node)

    g.set_entry_point("intake")
    g.add_edge("intake", "gherkin_specialist")
    g.add_edge("gherkin_specialist", "nfr_specialist")
    g.add_edge("nfr_specialist", "cai_gate")
