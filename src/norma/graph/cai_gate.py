"""
CAI Gate node — Constitutional AI validation of Gherkin artefacts.

Two assertions must pass before the artefact is emitted:
  1. Structural parse (non-LLM): Feature keyword + Scenario + Given/When/Then present.
  2. LLM rubric (cloud/claude-sonnet): coverage of time-of-day greeting, both content
...
def _assertion_2_rubric(gherkin: str, client: httpx.Client) -> tuple[bool, str]:
...
def cai_gate_node(state: NormaState) -> NormaState:
    ...
    with langfuse.start_as_current_observation(
        ...
        # Assertion 1 — structural (no LLM call)
        a1_ok, a1_msg = _assertion_1_structural(gherkin)

        if not a1_ok:
            feedback = f"Structural check failed: {a1_msg}"
        ...
        # Assertion 2 — LLM rubric
        with httpx.Client(timeout=60.0) as client:
            a2_ok, a2_msg = _assertion_2_rubric(gherkin, client)

        if not a2_ok:
            feedback = f"Coverage rubric failed: {a2_msg}"
